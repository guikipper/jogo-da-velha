...
function checkWinner() {
  for (let i = 0; i < POSITIONS.length; i++) {
    //percorre todas as possibilidades de vitória
    const [pos1, pos2, pos3] = POSITIONS[i]; //armazena as possibilidades atuais em variaveis separadas
    if (
      position_X.includes(pos1) &&
      position_X.includes(pos2) &&
      position_X.includes(pos3)
    ) {
      //verifica se X possui três posições vencedoras
      console.log("Ganhou A");
      winner.innerHTML = "Jogador X venceu";
      reset();
    } else if (
      position_Y.includes(pos1) &&
      position_Y.includes(pos2) &&
      position_Y.includes(pos3)
    ) {
      //verifica se O possui três posições vencedoras
